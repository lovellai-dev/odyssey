#!/usr/bin/env python3
"""Cosmos 3 (WAM) evaluation recipe for the LIBERO benchmark (single-agent pilot).

Sibling of ``pi05_libero_eval.py``: same subprocess + policy-server pattern and
the same LIBERO (robosuite/MuJoCo) env layer, but the pilot is a **Cosmos 3
world-action model** served by cosmos-framework's HTTP
``action_policy_server_libero``. ``LiberoRunner`` launches this script
(``pilot: cosmos3``) under the Odyssey interpreter, owns the ``ODYSSEY_*``
stdout protocol, cancellation and scoring; this script owns the Cosmos3 <->
LIBERO recipe: build the env, drive the pilot, replay the returned action
*chunk* open-loop, and report each episode.

Like π0.5 (and unlike GR00T's ``--serve_checkpoint``), Cosmos 3 is
**externally-served**: start the server yourself in the cosmos-framework env ::

    python -m cosmos_framework.scripts.action_policy_server_libero \
        --checkpoint-path <hf-id-or-exported-model-dir> --port 8000

and point this recipe at ``--host``/``--port``. ``--checkpoint`` is recorded in
the summary but NOT loaded here — the server already holds the weights, and it
also owns action de-normalization, prompt augmentation and diffusion sampling.
Any family member works (Edge/Nano/Super, DROID- or LIBERO-SFT'd): the
variant-specific knobs travel as flags (``--domain_name``, ``--image_size``,
``--n_action_steps``), and ``--n_action_steps 0`` (the default) adopts the
server's own resolved ``action_chunk_size`` via ``GET /info``.

Chunk replay is NOT hand-rolled here: ``make_cosmos3_pilot`` wraps the HTTP
client in the pilot-agnostic ``ChunkPilotAdapter`` (buffer + replay + flush-on-
instruction-change), so this loop just calls ``pilot.act`` once per env step
(``runners/agents/chunk_pilot.py``).

Launch contract (built by ``LiberoRunner`` for ``pilot: cosmos3``)::

    python cosmos3_libero_eval.py \
        --task <suite> --num_episodes <N> --checkpoint <path> \
        --task_id 0 --host H --port P [--n_action_steps 16 --domain_name libero ...]
...
Heavy deps (numpy, libero, the sibling transforms) are imported lazily in the
run path so the module imports under the bare stdlib and its argv + protocol
surface stay unit-testable on a CPU box.
"""
...
# When launched as a script (python …/runners/evals/cosmos3_libero_eval.py),
# sys.path[0] is THIS file's directory — which also holds libero.py (the odyssey
# LiberoRunner). That shadows the real LIBERO namespace package, so
# `from libero.libero import …` fails with "'libero' is not a package". Drop this
# dir; odyssey's own modules still import via the installed (editable) package,
# not the script dir. (Mirrors pi05_libero_eval.py / gr00t_libero_eval.py.)
_HERE = os.path.dirname(os.path.abspath(__file__))
sys.path[:] = [p for p in sys.path if os.path.abspath(p or ".") != _HERE]

log = logging.getLogger("cosmos3_libero_eval")
...
# eval recipe owns its protocol output (mirrors pi05_libero_eval.py).
...
def build_parser() -> argparse.ArgumentParser:
    """Argv per the LIBERO launch contract + Cosmos3 passthrough config."""
    ap = argparse.ArgumentParser(description="Cosmos 3 (WAM) policy eval on LIBERO.")
    # --- contract flags ---
    ...
    ap.add_argument("--checkpoint", default="",
                    help="Cosmos3 checkpoint id/path. Recorded in the summary only — "
                         "the policy server (pre-started) already holds the weights.")
    ...
    # --- Cosmos3 policy server + action-chunk config ---
    ap.add_argument("--host", default="127.0.0.1",
                    help="action_policy_server_libero host.")
    ap.add_argument("--port", type=int, default=8000,
                    help="action_policy_server_libero port.")
    ap.add_argument("--n_action_steps", type=int, default=0,
                    help="steps replayed per chunk before re-querying; 0 adopts the "
                         "server's resolved action_chunk_size from GET /info.")
    ap.add_argument("--domain_name", default="libero",
                    help="Cosmos3 action domain tag sent with each /predict.")
    ap.add_argument("--image_size", type=int, default=256,
                    help="server-side target height for the concat_view frame.")
...
# ---------------------------------------------------------------------------
# Env-coupled obs glue (numpy; imported lazily)
# ---------------------------------------------------------------------------
...
def _raw_obs(obs, *, image_key, wrist_image_key, flip) -> dict:
    """Shape a LIBERO env observation into ``make_cosmos3_pilot``'s ``raw_obs`` kwargs.

    ``ChunkPilotAdapter``'s injected ``observation_builder`` calls
    ``build_cosmos3_predict_request(instruction=..., **raw_obs)``, so the keys
    here MUST match its signature: image / wrist_image. The WAM is
    video-conditioned — no proprio state on the wire (contrast π0.5/GR00T).
    Images are pre-flipped to the pilot's orientation, matching the GR00T-LIBERO
    recipe.
    """
    return {
...
def run_eval(args: argparse.Namespace) -> dict:
    ...
    # Externally-served: the cosmos-framework server is pre-started and already
    # holds the weights; make_cosmos3_pilot wraps its HTTP client in the
    # ChunkPilotAdapter (n_action_steps=None -> adopt the server's chunk size).
    log.info("connecting to Cosmos3 policy server at %s:%d (checkpoint=%r served externally)",
             args.host, args.port, args.checkpoint)
    pilot = make_cosmos3_pilot(
        host=args.host,
        port=args.port,
        n_action_steps=args.n_action_steps or None,
        domain_name=args.domain_name,
        image_size=args.image_size,
        translation_only=args.translation_only,
    )
    log.info("chunk replay horizon: %d steps", pilot.n_action_steps)
    ...
    for ep in range(1, args.num_episodes + 1):
        ...
        try:
            while step < args.max_steps_per_episode and not success:
                ...
                # only re-queries the server when it drains (every n_action_steps).
                action = pilot.act(
                    _raw_obs(
                        obs,
            ...
            # A flaky /predict or env.step() must not abort the whole sweep (that
