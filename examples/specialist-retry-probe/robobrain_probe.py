"""Probe a served RoboBrain 2.5 as a SPECIALIST candidate (retry strategy).
...
Map v0.5 bake-off: retry strategy is its native benchmark family (mistake
existence / classification / recovery), challenged against the Cosmos3-Nano
Reasoner probed by ``examples/cosmos3-reasoner-probe``. This is a copy of that
probe with the serving glue adapted — the four questions are kept VERBATIM so
the two arms answer the same paper: any verdict delta is the model, not the
prompt.

It exercises ``OpenAICompatCompletionJudge`` — the exact ``CompletionDetector``
surface the multi-agent runtimes gate on — against a *vanilla* vLLM endpoint
(RoboBrain 2.5 is Qwen3-VL-8B-based; no vLLM-Omni, no ``modalities`` routing
knob needed). Four YES/NO questions per sampled frame:
...
* ``grasp``      — "is the object currently secured in the gripper?"  Expect
  NO on pre-grasp / post-place frames and YES only while the object is held.
* ``completion`` — the judge's stock done-check. Expect YES only at the end of
  successful rollouts.
* ``retry``      — **the headline check for this arm**: "is the robot
  failed/stuck such that it should abort and retry?"  Expect NO throughout
  successful rollouts and YES only late in failed rollouts.

Frames are sampled at fractional positions through each rollout MP4. Metric-
only: rates + latency + per-frame verdicts, no fabricated success_rate
(position ground truth is qualitative).
...
# The four templates are copied VERBATIM from the Cosmos3-Nano probe
# (examples/cosmos3-reasoner-probe/reasoner_probe.py) — a fair bake-off asks
# both arms the same questions. Do not "improve" the phrasing here without
# re-running the Cosmos arm on the same text.
CONTROL_TEMPLATE = (
