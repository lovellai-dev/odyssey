"""Watch the RoboBrain arm interrogate the SPECIALIST, frame by frame.

RoboBrain arm of the OBJECT-VERIFICATION bake-off — the model never sees the
*video*, it sees isolated frames, and per frame each object is one independent
image+text HTTP call ("is a <object> visible?"). This tool replays that loop
over a single rollout MP4 and shows it:

* terminal — one table row per (frame, object) as each answer arrives;
* HTML report — the playable video (slow-motion by default), live answer
  badges, per-object timeline bands with a synced playhead, and the Q/A table.
  The page loads ONCE and polls a sidecar ``*_data.js`` file, so new rows stream
  in with no page reload — playback is never interrupted. Clicking a timeline
  band or a table row seeks the video.

The SAM arm has its own viewer, ``visualize_sam_probe.py`` — it overlays the
returned masks/boxes instead of YES/NO badges, since SAM is concept
segmentation, not a chat judge.

Usage (server recipe in ../README.md; tunnel with `ssh -L 8002:127.0.0.1:8002`
if the model is served on the H100):

    python utils/visualize_probe.py \\
...
<title>Object-verification probe — {html.escape(video_path.name)}</title>
...
<h2>Object-verification probe — {html.escape(video_path.name)}
...
def main() -> None:
    ...
    names = [name for name, _, _ in questions]
    ...
    judges = {
        ...
        for name, template, _query in questions
    }
    queries = {name: query for name, _template, query in questions}
    prompts = {
        name: template.format(instruction=query)
    ...
    print(f"{'frame':>6} {'question':>22}  answer  latency")
    for index in indexes:
        ...
        for position, name in enumerate(names):
            ...
            answer = "YES" if judges[name].is_complete(frame, queries[name]) else "NO"
            ...
            print(f"{index:6d} {name:>22}  {answer:<6} {elapsed:.2f}s")
        write_data(out, records, done=False)
